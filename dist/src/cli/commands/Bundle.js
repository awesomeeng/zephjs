...
(() => {
    ...
    Object.keys(AwesomeUtils.MimeTypes.mimetypes).forEach((type) => {
        ...
        if (flavor === "image")
            image.concat(AwesomeUtils.MimeTypes.getExtensionForType(type).split(" "));
        if (flavor === "audio")
            audio.concat(AwesomeUtils.MimeTypes.getExtensionForType(type).split(" "));
        if (flavor === "video")
            video.concat(AwesomeUtils.MimeTypes.getExtensionForType(type).split(" "));
    });
...
class Bundle extends AwesomeCLI.AbstractCommand {
    ...
    execute(args, options) {
        ...
        if (args.length === 0) {
            ...
            let source = args[0];
            if (!source)
                return console.error("You must provided a source filename.");
            source = Path.resolve(process.cwd(), source);
            if (!source.endsWith(".js"))
                source += ".js";
            let target = args[1];
            if (!target)
                return console.error("You must provided a target filename.");
    ...
    rollup(source, target, quiet, full) {
        ...
        const resolvePlugin = {
            ...
            resolveId(source) {
                if (source.match(/(\/|\\)[Zz]eph(\.(min|full))?\.js$/))
                    return zeph;
                return null;
            }
        };
    ...
    return new Promise(async (resolve, reject) => {
        try {
            if (url.startsWith("http:") || url.startsWith("https:") || url.startsWith("ftp:")) {
                ...
                let response = null;
                while (true) {
                    if (tried.indexOf("" + url) > -1)
                        return reject("URL redirect created a circular reference and could not be resolved.");
                    tried.push(url);
                    ...
                    if (response.statusCode === 200)
                        ...
                        return reject("URL returned an error code: " + response.statusCode + ": " + (response.status || ""));
                }
                if (!response)
                    return reject("URL did not return anything.");
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (typeof node === "object" && node.type && node.type === "CallExpression" && node.callee && node.callee.type === "Identifier") {
                    let revised = null;
                    if (node.callee.name === "html")
                        revised = await inlineHTML(root, code, offset, node, quiet);
                    else if (node.callee.name === "css")
                        revised = await inlineCSS(root, code, offset, node, quiet);
                    else if (node.callee.name === "asset")
                        revised = await inlineAsset(root, code, offset, node, quiet);
                    if (revised) {
                        code = revised.code;
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (!arg)
                return resolve();
            if (!arg.type)
                return reject("html() statement was not syntactically valid.");
            if (arg.type === "TemplateLiteral")
                return resolve(); // template literals are allowed for raw content.
            if (arg.type !== "Literal")
                return reject("html() statement may only be a string literal.");
            if (!arg.value)
                return resolve();
            let start = arg.start;
            let end = arg.end;
            ...
            if (data === undefined || data === null)
                return reject("Could not find inline reference to " + filename + ".");
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (!arg.type || !arg.value)
                return reject("css() statement was not syntactically valid.");
            if (arg.type === "TemplateLiteral")
                return resolve(); // template literals are allowed for raw content.
            if (arg.type !== "Literal")
                return reject("css() statement may only be a string literal.");
            let start = arg.start;
            let end = arg.end;
            let length = arg.value.length;
            let filename = arg.value;
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (!arg)
                return resolve();
            if (!arg.type || !arg.value)
                return reject("asset() statement was not syntactically valid.");
            if (arg.type === "TemplateLiteral")
                return resolve(); // template literals are allowed for raw content.
            if (arg.type !== "Literal")
                return reject("asset() statement may only be a string literal.");
            let start = arg.start;
            let end = arg.end;
            let length = arg.value.length;
            let filename = arg.value;
            ...
            if (filename.startsWith("http:") || filename.startsWith("https:") || filename.startsWith("ftp:")) {
                ...
                ({ data, mimetype } = await loader(filename, filename, null));
                if (!mimetype)
                    mimetype = AwesomeUtils.MimeTypes.getTypeForExtension(Path.extname(filename));
            }
            if (!data)
                return reject("Could not find inline reference to " + filename + ".");
            if (!mimetype)
                return reject("asset() could not associate valid mime type with filename: " + filename);
...
const resolveFilename = function resolveFilename(root, filename, extensions) {
    ...
    if (AwesomeUtils.FS.existsSync(possible))
        return possible;
    if (!extensions)
        return null;
